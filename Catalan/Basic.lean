...
def plane_tree_nodes : plane_tree → ℕ
| .node [] => 0
| .node (tr :: []) => plane_tree_nodes tr + 1
| .node (tr :: trs) => plane_tree_nodes tr + plane_tree_nodes (plane_tree.node trs) + 1

def plane_tree_of_full_binary_tree :
  full_binary_tree → plane_tree
  | .leaf => plane_tree.node []
  | .join T1 T2 => plane_tree.node (plane_tree_of_full_binary_tree T1 :: plane_tree_of_full_binary_tree T2 :: [])
...
/-big task 4-/
def list_plane_tree_equiv_plane_tree : List plane_tree ≃ plane_tree :=
sorry

/-big task 5-/
def plane_tree_equiv_full_binary_tree : plane_tree ≃ full_binary_tree :=
sorry
